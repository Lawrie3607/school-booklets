...
const Dashboard: React.FC<{
  ...
  const refreshData = async () => {
    try {
      ...
      setAssignments(a || []);
      if (isSuperAdmin) {
          console.log('Dashboard: pulling users from Supabase before loading local cache...');
          try {
            await storageService.pullUsersFromRemote();
          } catch (pullErr) {
            console.warn('Dashboard: pullUsersFromRemote failed', pullErr);
          }
              ...
              {isSuperAdmin && (
                <div className="flex items-center gap-3 mt-4">
                  <button
                    ...
                    title="Users"
                    className="p-4 bg-indigo-600 text-white rounded-2xl hover:bg-black transition-all shadow-xl"
                  ><svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></button>
